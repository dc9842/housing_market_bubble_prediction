...
def download_2018():
    ...
    templateURL_file =  acsURL + "/{}_1yr_Summary_FileTemplates.zip".format(year)
    ...
    miniGeoURL_file = acsParentURL + "/{}/documentation/geography/1_year_Mini_Geo.xlsx".format(year)
    ...
    if(year ==2019):
        appendicesURL_file =  acsParentURL + "/{}/documentation/tech_docs/ACS_{}_SF_1YR_Appendices.xlsx".format(year, year)
        filePath = os.path.join(folderAdd,"ACS_{}_SF_1YR_Appendices.xlsx".format(year))
    else:
        appendicesURL_file =  acsParentURL + "/{}/documentation/tech_docs/ACS_{}_SF_1YR_Appendices.xls".format(year, year)
    ...
    if(year >2013):
        tableShellURL_file = acsParentURL + "/{}/documentation/user_tools/ACS{}_Table_Shells.xlsx".format(year, year)
    else:
        tableShellURL_file = acsParentURL + "/{}/documentation/user_tools/ACS{}_TableShells.xls".format(year, year)
    ...
    s = appData[appData["Table Number"]=="B24011"]
    if(s.shape[0]==1):
        sequenceNum = s["Summary File Sequence Number"].values[0]
        print(sequenceNum)
    ...
    fileName = os.path.join(folderAdd, "{}/{}_SFGeoFileTemplate.xlsx".format(summaryFileTemplates,year))
    geoFileNames = pd.read_excel(fileName)

    fileName = os.path.join(folderAdd,"summaryFileTemplates/seq{}.xlsx".format(sequenceNum))
